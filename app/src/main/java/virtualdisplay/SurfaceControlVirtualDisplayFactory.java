...
public class SurfaceControlVirtualDisplayFactory implements VirtualDisplayFactory {
    ...
    public VirtualDisplay create(String name, int width, int height, int dpi, int flags, Surface surface, Handler handler) {
        try {
            ...
            final Method getServiceMethod = Class.forName("android.android.os.ServiceManager").getDeclaredMethod("getService", new Class[]{String.class});
            ...
            return new VirtualDisplay() {
                ...
                class C04901 extends IRotationWatcher.Stub {
                    C04901() {
                    }

                    public void onRotationChanged(int rotation) throws RemoteException {
                        Point oldSize = SurfaceControlVirtualDisplayFactory.this.displaySize;
                        SurfaceControlVirtualDisplayFactory.this.displaySize = SurfaceControlVirtualDisplayFactory.getCurrentDisplaySize();
                        if (!oldSize.equals(SurfaceControlVirtualDisplayFactory.this.displaySize)) {
                            try {
                                Rect layerStackRect = new Rect(0, 0, SurfaceControlVirtualDisplayFactory.this.displaySize.x, SurfaceControlVirtualDisplayFactory.this.displaySize.y);
                                openTransactionMethod.invoke(null, new Object[0]);
                                setDisplayProjectionMethod.invoke(null, new Object[]{token, Integer.valueOf(0), layerStackRect, SurfaceControlVirtualDisplayFactory.this.displayRect});
                                closeTransactionMethod.invoke(null, new Object[0]);
                            } catch (Exception e) {
                                throw new AssertionError(e);
                            }
